...
<div class="container-form-wrapper">
    <div class="procuradores-form">
        <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data', 'id' => 'procuradores-form']]); ?>
        ...
        <div class="form-phases-container">
            <?php if ($isNewRecord): ?>
                ...
                <div class="form-phase active" id="phase-1">
                    ...
                    ])->textInput(['maxlength' => true])->label('<i class="fas fa-user"></i> Nombre del Procurador') ?>
                ...
                <div class="form-phase" id="phase-2" style="display:none;">
                    ...
                    <div class="form-group field-procuradores-resolucion required-field">
                        <label class="control-label" for="procuradores-resolucion"><i class="fas fa-file-alt"></i> Resolución</label>
                        <div class="input-group mb-3 resolution-input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text">PGEB-</span>
                            </div>
                            <?= Html::activeTextInput($model, 'resolucion', ['class' => 'form-control', 'id' => 'procuradores-resolucion', 'required' => true]) ?>
                ...
                <div class="form-phase" id="phase-3" style="display:none;">
                    ...
                    <div class="form-group field-procuradores-gaceta required-field">
                        <label class="control-label" for="procuradores-gaceta"><i class="fas fa-newspaper"></i> Gaceta</label>
                        <div class="input-group mb-3 gaceta-input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text">N°</span>
                            </div>
                            <?= Html::activeTextInput($model, 'gaceta', ['class' => 'form-control', 'id' => 'procuradores-gaceta', 'required' => true]) ?>
                            <div class="input-group-append">
                                <span class="input-group-text border-right-0">/</span>
                            </div>
                ...
                <div class="form-phase" id="phase-4" style="display:none;">
                    ...
                    <div class="form-group field-procuradores-firma required-field">
                        ...
                        <div class="help-block">Seleccione una imagen para la firma del procurador</div>
                    </div>
            ...
            <?php else: ?>
                ...
                <div class="form-phase active">
                    <div class="phase-subtitle">Actualizar Procurador</div>
                    
                    <?= $form->field($model, 'nombre')->textInput(['maxlength' => true])->label('<i class="fas fa-user"></i> Nombre del Procurador') ?>
                    
                    <div class="form-group field-procuradores-firma">
                        <label class="control-label" for="firma-upload"><i class="fas fa-signature"></i> Firma</label>
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            <img src="data:image/png;base64,<?= $model->firma_base64 ?>" alt="Firma del procurador" style="max-width: 300px; max-height: 100px;">
                        </div>
                    </div>
                    <?php endif; ?>
...
$script = <<<JS
    ...
    $('#firma-upload').change(function() {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                $('input[name="Procuradores[firma_base64]"]').remove();
                // Añadir nuevo input
                $('<input>').attr({
                    type: 'hidden',
                    name: 'Procuradores[firma_base64]',
                    value: base64
                }).appendTo('#procuradores-form');
            }
    ...
    // Función para convertir fecha de formato español a inglés
    function convertDateFormat(spanishDate) {
        if (!spanishDate) return '';
        
        // Dividir la fecha en sus componentes
        var parts = spanishDate.split('-');
        if (parts.length !== 3) return spanishDate;
        
        // Reorganizar al formato yyyy-MM-dd
        return parts[2] + '-' + parts[1] + '-' + parts[0];
    }
    
    // Manejar la concatenación de resolución y gaceta con año al enviar el formulario
    $('#procuradores-form').on('beforeSubmit', function() {
        ...
        // Procesar resolución
        var resolucion = $('#procuradores-resolucion').val();
        var resYear = $('#resolucion-year').val();
        
        if (resolucion && resYear) {
            $('#procuradores-resolucion').val(resolucion + '/' + resYear);
        }
        
        // Procesar gaceta
        var gaceta = $('#procuradores-gaceta').val();
        var gacYear = $('#gaceta-year').val();
        
        if (gaceta && gacYear) {
            $('#procuradores-gaceta').val(gaceta + '/' + gacYear);
        }

        // Convertir fechas de español a inglés
        var fechaResolucion = $('#procuradores-fecha_resolucion').val();
        var fechaGaceta = $('#procuradores-fecha_gaceta').val();

        // Convertir y actualizar las fechas en el formulario
        $('#procuradores-fecha_resolucion').val(convertDateFormat(fechaResolucion));
        $('#procuradores-fecha_gaceta').val(convertDateFormat(fechaGaceta));
